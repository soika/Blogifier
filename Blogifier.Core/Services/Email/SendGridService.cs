...
namespace Blogifier.Core.Services.Email
{
    ...
    public class SendGridService : IEmailService
    {
        ...
        public bool Enabled
        {
            get
            {
                return this.db.CustomFields.GetValue(CustomType.Application, 0, Constants.SendGridApiKey).Length > 0;
            }
        }
        ...
        public Task Send(string to, string subject, string message, Profile profile = null)
        {
            ...
            var apiKey = this.db.CustomFields.GetValue(CustomType.Application, 0, Constants.SendGridApiKey);
            if (string.IsNullOrEmpty(apiKey))
            {
                this.logger.LogError(Constants.SendGridNotConfigured);
                return Task.FromResult(false);
            }

            var admin = this.db.Profiles.Find(p => p.IsAdmin).FirstOrDefault();
            var sentFrom = admin.AuthorEmail;

            // if user has own API key, use it instead of app API key
            if (profile != null)
            {
                var userKey = this.db.CustomFields.GetValue(CustomType.Profile, profile.Id, Constants.SendGridApiKey);
